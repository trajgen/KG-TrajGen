...
def set_logger(log_dir="./logs/", log_prefix=""):
    ...
    fh = logging.FileHandler(f"{log_dir}/{ts}.log")
...
def read_rid_gps(data):
    ...
    if os.path.exists(rid_gps_file):
        ...
        rid_gps = {}
        rid_info = pd.read_csv(f"./data/{data}/porto.geo")
...
def read_od_pair_distribution(data):
    split = "tra"
    od_pair_file = f"./data/{data}/od_and_probs_float_50.pt"
    if os.path.exists(od_pair_file):
        od_and_probs_float = torch.load(od_pair_file)
    else:
        df = pd.read_csv(f"./data/{data}/traj_{split}_50.csv")
...
def read_node_feature_file(data="", device=""):
    ...
    if os.path.exists(node_feature_file):
        ...
        # normalization for continuous attribution  对连续属性进行归一化
        ...
        # one-hot encoding for discrete attribution 对离散属性进行直接编码
        ...
        node_features = node_features.drop(columns=onehot_list)   #重新删掉

        with open(f"./data/{data}/bj_rid_gps.json", "r") as f:
            rid_gps = json.load(f)
        lon_grid = []  # x
        lat_grid = []  # y
...
def my_collate_fn(indices, adj, device):
    trace_loc = []
    trace_tim = []
    trace_dur_time = []
    for i in indices:
        ...
        trace_dur_time.append(torch.tensor(i[2]))
    ...
    trace_dur_time = pad_sequence(trace_dur_time, batch_first=True, padding_value=0)
    ...
    return [
        ...
        trace_dur_time.to(device)
    ]


def generate_data_loader(city, split, batch_size=None, adj=None, device=None):
    if split == "tes":  #如果是测试集的话
        test_data = []
        df = pd.read_csv(f"./data/{city}/traj_{split}.csv") #读取测试集文件
        for index, row in tqdm(
            df.iterrows(), total=df.shape[0], desc="generate test data"
        ):
            traj_loc = list(map(int, row["rid_list"].split(",")))
            traj_tim = list(map(int, row["time_encode"].split(",")))
            traj_dur_tim = list(map(int, row["dur_time"].split(",")))
            test_data.append([traj_loc, traj_tim,traj_dur_tim])
        test_dataset = ListDataset(test_data)
        return DataLoader(
            test_dataset,
            batch_size=batch_size,
            shuffle=True,
            collate_fn=lambda b: my_collate_fn(b, adj, device),
        )
    elif split == "tra_and_val":
        data_tra = []
        df_tra = pd.read_csv(f"./data/{city}/traj_tra.csv")
        for index, row1 in tqdm(df_tra.iterrows(),total=df_tra.shape[0],desc="generate training data loader"):
            traj_loc = list(map(int, row1["rid_list"].split(",")))
            traj_tim = list(map(int, row1["time_encode"].split(",")))
            traj_dur_tim = list(map(int, row1["dur_time"].split(",")))
            data_tra.append([traj_loc, traj_tim,traj_dur_tim])
        tra_dataset = ListDataset(data_tra)

        data_val = []
        df_val = pd.read_csv(f"./data/{city}/traj_val.csv")

        for index, row in tqdm(df_val.iterrows(), total=df_val.shape[0], desc="generate valid data loader"):
            traj_loc = list(map(int, row["rid_list"].split(",")))
            traj_tim = list(map(int, row["time_encode"].split(",")))
            traj_dur_tim = list(map(int, row["dur_time"].split(",")))
            data_val.append([traj_loc, traj_tim,traj_dur_tim])
