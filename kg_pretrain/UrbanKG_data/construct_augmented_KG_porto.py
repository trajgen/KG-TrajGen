...
for i in tqdm(range(borough_dataframe.shape[0])):
    ...
    for j in range(borough_dataframe.shape[0]):
        ...
        if head_borough.touches(tail_borough):
            BNB.append('Borough/' + str(borough_dataframe.iloc[i].ID) + ' BNB ' + 'Borough/' + str(borough_dataframe.iloc[j].ID))
...
poi_dataframe = pd.read_csv('./Processed_data/Porto/porto_poi.csv')
...
road_dataframe = pd.read_csv('./Processed_data/Porto/porto_road_geo.csv')
...
junction_dataframe = pd.read_csv('./Processed_data/Porto/porto_junction.csv')
junction_datanumpy = np.array(junction_dataframe[[ "node_id", "borough_id", "area_id", "fclass"]])
...
road_dataframe = pd.read_csv('./Processed_data/Porto/porto_road_geo.csv')
junction_dataframe2 = pd.read_csv('./Processed_data/Porto/porto_junction.csv')
road_datanumpy = road_dataframe[['geo_id']].values
junction_datanumpy2 = junction_dataframe2[['node_id']].values
...
for i in tqdm(range(area_dataframe.shape[0])):
    ...
    for j in range(borough_dataframe.shape[0]):
        ...
        if area.within(borough) or area.intersects(borough):
            ALB.append('Area/' + str(area_dataframe.iloc[i].area_ID) + ' ALB ' + 'Borough/' + str(borough_dataframe.iloc[j].ID))
...
base_KG=[]
with open(r'./UrbanKG/porto_10904/new_KG/UrbanKG_base_porto_2.txt','r') as f1:
    for line in f1:
...
with open(r'./UrbanKG/porto_10904/new_KG/UrbanKG_augmented_porto.txt','w') as f2:
